...
export function ProfilePage({ user, token, onLogin, onLogout }: ProfilePageProps) {
  ...
  async function handleRegister(event: FormEvent<HTMLFormElement>) {
    ...
    const fileInput = form.querySelector<HTMLInputElement>('input[name="id_card"]');
    if (!fileInput?.files?.length) {
      setError("Загрузите фото Yessenov ID-карты");
      return;
    }
    ...
    try {
      ...
      setMessage(t("profile.msg.register_pending"));
    } catch (err: unknown) {
          ...
          <div className="m3-auth-card">
            ...
            {tab === "login" ? (
              ...
              <form className="m3-auth-actions" onSubmit={handleRegister}>
                ...
                <div style={{ background: "var(--md-surface-container)", borderRadius: "var(--md-radius-lg)", padding: "1rem", border: "1px solid var(--md-outline-variant)" }}>
                  <p style={{ fontWeight: 600, fontSize: "0.95rem", marginBottom: "0.75rem", color: "var(--md-on-surface)" }}>
                    📋 {t("profile.register.idcard_title")}
                  </p>
                  <div className="m3-text-field">
                    <label>{t("profile.register.full_name")}</label>
                    <input required name="full_name" minLength={3} placeholder={t("profile.register.full_name_placeholder")} />
                  </div>
                  <div className="m3-text-field">
                    <label>{t("profile.register.iin")}</label>
                    <input required name="iin" pattern="\d{12}" maxLength={12} minLength={12} placeholder="000000000000" inputMode="numeric" />
                    <span style={{ fontSize: "0.75rem", color: "var(--md-on-surface-variant)" }}>{t("profile.register.iin_hint")}</span>
                  </div>
                  <div className="m3-text-field">
                    <label>{t("profile.register.idcard_photo")}</label>
                    <input 
                      required 
                      type="file" 
                      name="id_card" 
                  ...
                  {idCardPreview && (
                    <div style={{ marginTop: "0.5rem", borderRadius: "var(--md-radius-md)", overflow: "hidden", border: "1px solid var(--md-outline-variant)" }}>
                      <img src={idCardPreview} alt="ID Card Preview" style={{ width: "100%", maxHeight: "200px", objectFit: "cover" }} />
                    </div>
                  )}
                </div>
        ...
        <div className="m3-profile-wrap">
          ...
          {user.verificationStatus && user.verificationStatus !== "approved" && (
            <div style={{
              ...
              border: `1px solid ${user.verificationStatus === "rejected" ? "#d32f2f" : "var(--md-outline-variant)"}`,
              background: user.verificationStatus === "rejected" ? "rgba(211, 47, 47, 0.08)" : "rgba(255, 193, 7, 0.08)",
              display: "flex", alignItems: "center", gap: "0.75rem",
            }}>
              <span style={{ fontSize: "1.5rem" }}>{user.verificationStatus === "rejected" ? "❌" : "⏳"}</span>
              <div>
                <p style={{ fontWeight: 600, margin: 0, fontSize: "0.9rem" }}>
                  {user.verificationStatus === "pending" ? "Заявка на проверке" : "Верификация отклонена"}
                </p>
                <p style={{ margin: 0, fontSize: "0.8rem", color: "var(--md-on-surface-variant)" }}>
                  {user.verificationStatus === "pending"
                    ? "Администратор проверит вашу ID-карту в ближайшее время"
                    : "Загрузите корректное фото ID-карты и попробуйте снова"}
                </p>
              </div>
            </div>
          )}
