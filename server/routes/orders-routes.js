...
ordersRouter.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const service = await get(
      "SELECT id, user_id, university FROM services WHERE id = $1",
      [serviceId]
    );
    if (!service) {
    ...
    const existing = await get(
      `
        ...
        WHERE service_id = $1 AND client_id = $2 AND status IN ('pending', 'accepted')
      `,
  ...
  asyncHandler(async (req, res) => {
    const rows = await all(
      `
        ...
        JOIN users provider ON provider.id = service_orders.provider_id
        LEFT JOIN service_reviews reviews ON reviews.order_id = service_orders.id
        WHERE service_orders.client_id = $1 OR service_orders.provider_id = $2
...
ordersRouter.patch(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (status === "under_review") {
      await run(
        "UPDATE service_orders SET status = $1, completed_at = CURRENT_TIMESTAMP WHERE id = $2",
        [status, orderId]
      );
    } else {
      await run("UPDATE service_orders SET status = $1 WHERE id = $2", [status, orderId]);
    }
  ...
  asyncHandler(async (req, res) => {
    ...
    const messages = await all(
      `
        ...
        WHERE service_messages.order_id = $1
...
ordersRouter.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const result = await run(
      "INSERT INTO service_messages (order_id, sender_id, message) VALUES ($1, $2, $3) RETURNING id",
...
ordersRouter.patch(
  ...
  asyncHandler(async (req, res) => {
    ...
    const unread = await all(
      "SELECT DISTINCT sender_id FROM service_messages WHERE order_id = $1 AND sender_id != $2 AND is_read = 0",
      [orderId, req.user.id]
    );

    if (unread.length > 0) {
      await run(
        "UPDATE service_messages SET is_read = 1 WHERE order_id = $1 AND sender_id != $2",
        [orderId, req.user.id]
      );
...
ordersRouter.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const existing = await get("SELECT id FROM service_reviews WHERE order_id = $1", [orderId]);
    if (existing) {
...
ordersRouter.patch(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (price > 0) {
      await run("UPDATE users SET balance = balance - $1 WHERE id = $2", [price, req.user.id]);
      await run(
        "INSERT INTO transactions (user_id, amount, type, description) VALUES ($1, $2, 'expense', $3)",
        [req.user.id, price, `Резерв средств по заказу #${orderId}`]
      );
    }

    await run(
      "UPDATE service_orders SET payment_status = $1, payment_paid_at = CURRENT_TIMESTAMP WHERE id = $2",
      [status, orderId]
    );
