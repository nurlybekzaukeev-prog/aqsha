...
// GET /api/ads/:id/chats
adChatRouter.get(
  "/ads/:id/chats",
  authMiddleware,
  asyncHandler(async (req, res) => {
    const adId = parsePositiveInt(req.params.id, "id");
    const ad = await get("SELECT id, user_id FROM ads WHERE id = $1", [adId]);
    ...
    const chats = await all(`
      SELECT 
        m.client_id AS id,
        u.name as name,
        COUNT(CASE WHEN m.is_read = 0 AND m.sender_id != $1 THEN 1 END) as unread_count,
        MAX(m.created_at) as last_message_at
      FROM ad_messages m
      JOIN users u ON m.client_id = u.id
      WHERE m.ad_id = $2 AND m.client_id IS NOT NULL
      GROUP BY m.client_id, u.name
...
adChatRouter.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const ad = await get("SELECT id, user_id, title FROM ads WHERE id = $1", [adId]);
    if (!ad) {
    ...
    let queryArgs = [adId];
    let paramIdx = 2;
    let queryCond = "WHERE ad_messages.ad_id = $1";
    if (clientId) {
      queryCond += ` AND ad_messages.client_id = $${paramIdx++}`;
  ...
  asyncHandler(async (req, res) => {
    ...
    if (isOwner) {
      ...
      const participation = await get(
        "SELECT id FROM ad_messages WHERE ad_id = $1 AND client_id = $2 AND sender_id != $3 LIMIT 1",
    ...
    const result = await run(
      "INSERT INTO ad_messages (ad_id, sender_id, message, client_id) VALUES ($1, $2, $3, $4) RETURNING id",
...
adChatRouter.patch(
  ...
  asyncHandler(async (req, res) => {
    ...
    const unread = await all(
      "SELECT DISTINCT sender_id FROM ad_messages WHERE ad_id = $1 AND client_id = $2 AND sender_id != $3 AND is_read = 0",
      [adId, clientId, req.user.id]
    );

    if (unread.length > 0) {
      await run(
        "UPDATE ad_messages SET is_read = 1 WHERE ad_id = $1 AND client_id = $2 AND sender_id != $3",
        [adId, clientId, req.user.id]
      );
  ...
  asyncHandler(async (req, res) => {
    const rows = await all(
      `
        ...
        GROUP BY ads.id, ads.title, ads.status, ads.created_at, users.name
        ORDER BY last_message_at DESC
        LIMIT 100
      `
    );
