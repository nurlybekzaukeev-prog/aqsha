...
servicesRouter.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    let sql = `
      ...
      WHERE services.university = $${paramIdx++}
    `;

    if (search) {
      sql += ` AND (LOWER(services.title) LIKE $${paramIdx} OR LOWER(services.description) LIKE $${paramIdx + 1})`;
    ...
    if (category) {
      sql += ` AND services.category = $${paramIdx++}`;
      params.push(category);
    }

    if (minPrice !== null) {
      sql += ` AND services.price >= $${paramIdx++}`;
    ...
    if (minRating !== null) {
      sql += ` AND COALESCE(ratings.rating_avg, 0) >= $${paramIdx++}`;
    ...
    sql += ` ORDER BY ${orderBy} LIMIT $${paramIdx++}`;
    params.push(limit);
  ...
  asyncHandler(async (req, res) => {
    ...
    const reviews = await all(
      `
        ...
        WHERE service_reviews.service_id = $1
...
servicesRouter.delete(
  ...
  asyncHandler(async (req, res) => {
    ...
    const service = await get("SELECT id, user_id FROM services WHERE id = $1", [serviceId]);
    if (!service) {
      throw notFound("Профиль не найден");
    }
...
servicesRouter.patch(
  ...
  asyncHandler(async (req, res) => {
    ...
    try {
      ...
      await run(
        `UPDATE services SET title = $1, category = $2, price = $3, description = $4, contact_phone = $5, contact_whatsapp = $6, contact_telegram = $7 WHERE id = $8`,
        [title, category, price, description, phone, whatsapp, telegram, serviceId]
      );

      if (req.files?.length) {
        const oldImages = await all("SELECT url FROM service_images WHERE service_id = $1", [serviceId]);
        await run("DELETE FROM service_images WHERE service_id = $1", [serviceId]);
        ...
        for (const file of req.files) {
          const url = `/uploads/${file.filename}`;
          await run("INSERT INTO service_images (service_id, url) VALUES ($1, $2)", [serviceId, url]);
        }
      }
