...
authRouter.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const iin = requireText(req.body.iin, "ИИН", { min: 12, max: 12 });
    const fullName = requireText(req.body.full_name, "Полное имя", { min: 3, max: 150 });

    if (!/^\d{12}$/.test(iin)) {
      throw badRequest("ИИН должен содержать ровно 12 цифр");
    }

    if (!req.file) {
      throw badRequest("Фото ID-карты обязательно");
    }
    ...
    const existingIin = await get("SELECT id FROM users WHERE iin = $1", [iin]);
    if (existingIin) {
      throw badRequest("Пользователь с таким ИИН уже существует");
    }

    const idCardUrl = `/uploads/${req.file.filename}`;
    const passwordHash = await bcrypt.hash(password, 10);
    const result = await run(
      `INSERT INTO users (name, email, university, password_hash, iin, full_name, id_card_url, verification_status)
       VALUES ($1, $2, $3, $4, $5, $6, $7, 'pending') RETURNING id`,
      [name, email, DEFAULT_UNIVERSITY, passwordHash, iin, fullName, idCardUrl]
    );
...
authRouter.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const storedCredential = await get(
      "SELECT user_id, credential_id, public_key, counter FROM webauthn_credentials WHERE credential_id = $1",
      [credentialId]
    );
    if (!storedCredential) {
    ...
    const challengeRow = await get(
      "SELECT id, challenge, expires_at FROM webauthn_challenges WHERE user_id = 0 AND type = 'login' AND challenge = $1 ORDER BY id DESC LIMIT 1",
      [receivedChallenge]
    );
    if (!challengeRow || isExpired(challengeRow.expires_at)) {
    ...
    await run("DELETE FROM webauthn_challenges WHERE id = $1", [challengeRow.id]);

    const user = await get("SELECT * FROM users WHERE id = $1", [storedCredential.user_id]);
    if (!user) {
...
authRouter.get(
  ...
  asyncHandler(async (req, res) => {
    const row = await get(
      "SELECT COUNT(*) as count FROM face_descriptors WHERE user_id = $1",
      [req.user.id]
    );
    res.json({ registered: Number(row?.count) > 0 });
  })
...
authRouter.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    await run("DELETE FROM face_descriptors WHERE user_id = $1", [req.user.id]);
...
authRouter.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const user = await get("SELECT * FROM users WHERE id = $1", [bestMatch.user_id]);
    if (!user) {
      throw unauthorized("Пользователь не найден");
    }
