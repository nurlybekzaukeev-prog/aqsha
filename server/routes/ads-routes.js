...
adsRouter.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    let sql = `
      ...
      WHERE ads.university = $${paramIdx++}
      AND ads.status = 'active'
    `;

    if (search) {
      sql += ` AND (LOWER(ads.title) LIKE $${paramIdx} OR LOWER(ads.description) LIKE $${paramIdx + 1})`;
    ...
    if (category) {
      sql += ` AND ads.category = $${paramIdx++}`;
      params.push(category);
    }

    if (minPrice !== null) {
      sql += ` AND ads.price >= $${paramIdx++}`;
    ...
    try {
      ...
      if (req.files?.length) {
        for (const file of req.files) {
          ...
          imageUrls.push(url);
          await run("INSERT INTO images (ad_id, url) VALUES ($1, $2)", [adResult.lastID, url]);
        }
      } else {
...
adsRouter.patch(
  ...
  asyncHandler(async (req, res) => {
    ...
    await run("UPDATE ads SET status = $1 WHERE id = $2", [status, adId]);
    res.json({ ok: true });
  })
);
